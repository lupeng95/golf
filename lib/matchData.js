...
last5Data = new Mongo.Collection('last5');
...
function addStat(newMatch){
  ...
  var last5 = matchData.find({userID:uid,valid:1},{fields:{summary:1}}).fetch();
  if(last5.length == 0){
    return;

  }

  var l5 = {userID:uid,total:0,push:0,on:0,son:0};
  var ll5 = last5Data.findOne({userID:uid})

  if(ll5){
    l5=ll5
  }
  var matchNum = last5.length;
  var t=0;
  var p=0;
  var o=0;
  var s=0;
  for (var i=0; i<matchNum;i++){
    t+=last5[i].summary.total;
    p+=last5[i].summary.push;
    o+=last5[i].summary.onRate;
    s+=last5[i].summary.sOnRate;
  }
  l5.total = parseInt(t/matchNum);
  l5.push = parseInt(p/matchNum);
  l5.on = parseInt(o/matchNum);
  l5.son = parseInt(s/matchNum);

  if(l5._id){
    last5Data.update(l5._id,l5)
  }else{
    last5Data.insert(l5);
  }
